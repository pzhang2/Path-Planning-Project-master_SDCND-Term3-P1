...
// front_car_dist check for all the cars in front in all 3 lanes.
// side_car_dist check for the cars in behind which might be too close for ego car to change lane.
bool too_close(double vehicle_s, double car_s, double front_car_dist, double side_car_dist) {
  if (((vehicle_s - car_s) > front_car_dist) or ((vehicle_s - car_s) < side_car_dist)){
    return false;
  } else {
    return true;
  }
...
double collisionCost (vector<double> lane_s, double car_s, double front_car_dist, double side_car_dist) {
  double collisionCost = 0;
  for(int i=0; i<lane_s.size(); i++){
    bool collision_potential = too_close(lane_s[i], car_s, front_car_dist, side_car_dist);
...
//                vehicle_s += vehicle_speed*0.02*previous_path_x.size();
          ...
          for (int i=0; i< 50-previous_path_x.size(); i++){
            double N = target_dist/(0.02*ref_vel/2.24);  // transfer from mile/hr to m/sec
